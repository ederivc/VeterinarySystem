...
def insert_user(db, request):
    ...
    try:
            ...
            if "status" in request and request["status"] == "Administrador":
                if not request["hash"] == HASH:
                    response = make_response(
                        jsonify(
                            message="No tienes permiso para registrarte como administrador"
                        ),
                        400,
                    )
                    abort(response)
        ...
        if re.match(EMAIL_REGEX, email):
            ...
            response = make_response(
                jsonify(message="Datos incorrectos, verificalos nuevamente"),
                400,
            )
            abort(response)
...
def create_user():
    if request.method == "POST":
        ...
        if db_email is None and db_phone is None:
            ...
            response = make_response(
                jsonify(
                    message="Ya existe un usuario registrado con ese correo y teléfono"
                ),
                400,
            )
            abort(response)
        ...
        if not g.user:
            return {
                "Response": "Has sido registrado correctamente, verifica tu correo para activar tu cuenta"
            }
        else:
...
def update_user_route():
    if request.method == "POST":
        ...
        if g.user["user_id"] == request.json["id"]:
            response = make_response(
                jsonify(message="No puedes modificarte a ti mismo"), 400
            )
            abort(response)
        else:
            db = get_db()
            ...
            if db_phone is None:
                ...
                response = make_response(
                    jsonify(message="El número de teléfono ya se encuentra registrado"),
                    400,
                )
                abort(response)
    ...
    if request.method == "POST":
        ...
        if contraseña != "" and new_contraseña != "" and new_confirm_contraseña != "":
            if new_contraseña != new_confirm_contraseña:
                response = make_response(
                    jsonify(message="Las contraseñas nuevas no coinciden"), 400
                )
                abort(response)
            ...
            if not check_password_hash(user_request["password"], contraseña):
                ...
                db.execute(
                    ...
                    (
                        ...
                        apellidos,
                        telefono,
                        generate_password_hash(new_contraseña),
                        request.json["id"],
                    ),
                )
                db.commit()
...
def delete_user(id):
    if g.user["user_id"] == id:
        response = make_response(
            jsonify(message="No puedes eliminarte a ti mismo"), 400
        )
        abort(response)
    else:
        db = get_db()

        user_in_appointment = db.execute(
            "SELECT * FROM AppointmentUser WHERE user_id = ?", (id,)
        ).fetchall()

        user_in_pedido = db.execute(
        ...
        if len(user_in_appointment) == 0 and len(user_in_pedido) == 0:
            ...
            response = make_response(
                jsonify(
                    message="No puedes eliminar un usuario asociado con una cita o pedido"
                ),
                400,
            )
            abort(response)
