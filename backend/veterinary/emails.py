...
def confirm_email(token, id):
    ...
    db = get_db()
    db.execute("UPDATE User SET active = ? WHERE user_id = ?", (1, id))
    db.commit()
...
def send_email(email, token, id):
    ...
    msg["To"] = email
    msg.set_content(
        f"""Se ha realizado un registro en la plataforma de la veterinaria Valladolid con esta cuenta, para activarla de click en el siguiente link:
    ...
    try:
        ...
        response = make_response(
            jsonify(message="Datos incorrectos, verificalos nuevamente"), 400
        )
        abort(response)
